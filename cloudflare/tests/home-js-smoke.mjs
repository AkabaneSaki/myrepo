...
assert.equal((fragments.homeModalsScript.match(/project-form-section-head--plain/g) || []).length, 3);
assert.doesNotMatch(fragments.homeModalsScript, /project-form-section-head\"><div><h3/);
assert.match(fragments.homeModalsScript, /character-reference-modal/);
assert.match(fragments.homeModalsScript, /label for=\"characterReferenceName\"/);
assert.match(fragments.homeModalsScript, /name=\"worldbookFile\"/);
assert.match(fragments.homeModalsScript, /name=\"regexFile\"/);
assert.match(homeStylesSource, /\.project-form-section-head--plain \{ grid-template-columns:minmax\(0,1fr\); gap:0; \}/);
assert.match(homeStylesSource, /\.character-reference-modal \.modal-content \{ width:min\(94vw,980px\)/);
assert.match(homeStylesSource, /\.character-reference-modal \.project-form-section-head--plain \{ grid-template-columns:minmax\(0,1fr\);/);
