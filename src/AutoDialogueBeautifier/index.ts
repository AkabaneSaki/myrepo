...
const REGEX_JSON_URLS = [
  "https://cdn.jsdelivr.net/gh/AkabaneSaki/myrepo@2.0.7/regex.json",
  "https://gcore.jsdelivr.net/gh/AkabaneSaki/myrepo@2.0.7/regex.json",
  "https://testingcf.jsdelivr.net/gh/AkabaneSaki/myrepo@2.0.7/regex.json",
];
...
$(async () => {
  ...
  try {
    ...
    for (let i = 0; i < REGEX_JSON_URLS.length; i++) {
      ...
      try {
        ...
        console.info(
          `自适应正则: 成功从第 ${i + 1} 个URL加载 ${regexData.length} 条规则`,
        );
        loaded = true;
        break;
      } catch (urlError) {
  ...
  let cachedPatterns:
    | { scriptName: string; pattern: RegExp; quickCheck: string }[]
    | null = null;
  // 缓存启用的正则规则
  ...
  const getEnabledRegexRules = (): TavernRegex[] => {
    ...
    cachedEnabledRules = regexData
      ...
      .map((rule: any) => ({
        ...
        trim_strings: Array.isArray(rule.trimStrings)
          ? rule.trimStrings.join("\n")
          : "",
        source: {
          user_input: false,
          ai_output: true,
      ...
      .map((rule: any) => {
        ...
        try {
          ...
          if (patternStr.includes("{") || patternStr.includes("\\{")) {
            ...
            quickCheck = patternStr.substring(
              0,
              Math.min(15, patternStr.length),
            );
          }
  ...
  const detectNeededScriptNames = (
    messageContent: string,
    skipStoredNames: string[] = [],
  ): string[] => {
  ...
  const saveStoredRegexNames = (names: string[]): void => {
    try {
      ...
      insertOrAssignVariables(
        { [VARIABLE_KEY]: sortedNames },
        { type: "chat" },
      );
      console.info(
        `自适应正则: 已保存 ${sortedNames.length} 个正则名称到聊天变量`,
      );
    } catch (e) {
  ...
  const getCharacterCardRegexNames = async (): Promise<Set<string>> => {
    ...
    try {
      const regexes = await updateTavernRegexesWith(
        (regexes: TavernRegex[]) => {
  ...
  const registerRegexRule = async (rule: TavernRegex): Promise<void> => {
    try {
      await updateTavernRegexesWith((regexes: TavernRegex[]) => {
        // 避免重复注册同名正则
        const filtered = regexes.filter(
          (r: TavernRegex) => r.script_name !== rule.script_name,
        );
  ...
  const syncRegexWithVariable = async (): Promise<void> => {
    ...
    try {
      ...
      const enabledRegexNames = new Set(
        getEnabledRegexRules().map((r) => r.script_name),
      );
      ...
      if (toRemove.length > 0) {
        ...
        console.info(
          `自适应正则: 已移除 ${toRemove.length} 条不在变量列表中的规则: ${toRemove.join(", ")}`,
        );
      }
    ...
    try {
      await updateTavernRegexesWith((regexes: TavernRegex[]) => {
        return regexes.filter(
          (r: TavernRegex) => !names.includes(r.script_name),
        );
      });
  ...
  const scanAndUpdateVariable = async (): Promise<void> => {
    try {
      ...
      // 检测需要的正则名称（跳过已存储的正则）
      const detectedNames = detectNeededScriptNames(
        messageContent,
        storedNames,
      );
      ...
      if (storedSorted !== combinedSorted) {
        console.info(
          `自适应正则: 检测到变化，从 ${storedNames.length} 个更新到 ${combinedNames.length} 个`,
        );
  ...
  const initializeFromVariable = async (): Promise<void> => {
    ...
    if (storedNames.length > 0) {
      console.info(
        `自适应正则: 初始化，从变量加载 ${storedNames.length} 个正则`,
      );
      await syncRegexWithVariable();
    } else {
      // 如果变量为空，扫描当前消息
  ...
  const watchVariableChange = (): void => {
    ...
    (window as any).replaceVariables = function (
  ...
  eventOn(tavern_events.MESSAGE_RECEIVED, async (messageId: number) => {
    // 检查是否已经处理过此消息
    const lastProcessedId = getVariables({ type: "chat" })[
      LAST_PROCESSED_MESSAGE_ID_KEY
    ];
    if (
      lastProcessedId !== undefined &&
      lastProcessedId !== null &&
      String(lastProcessedId) === String(messageId)
    ) {
    ...
    try {
      insertOrAssignVariables(
        { [LAST_PROCESSED_MESSAGE_ID_KEY]: messageId },
        { type: "chat" },
      );
